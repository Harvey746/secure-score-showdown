...
export const GameBoard = ({ gameState, onCardFlip, onResolveMatch, onStartNewGame, isLoading, pendingResolution }: GameBoardProps) => {
  ...
  // Update card states based on game state
  useEffect(() => {
    setCardStates(prevStates => {
      const newStates = [...prevStates];

      // Reset all cards to face down (except matched ones)
      ...
      // Show currently flipped cards (only if not matched)
  ...
  }, [gameState.flippedCard1, gameState.flippedCard2, matchedCards]);
